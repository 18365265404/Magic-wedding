...
//			alert()
		...
		case "WEDDING_DESIGN"://婚礼设计
			...
			for(let i=0;i<list.length;i++){
				if(list[i].dicValue===type_list.typeDesign){
					
					$(".scheme-title-list3").find("li").eq(i).css({"color":"red"})
				}
...
function showDesign(tag,value,page){
	...
	success: function(data) {
		...
		let wentuStr=""
		    for(let i=0;i<wentuList.length;i++){
		    	wentuStr+=`
					<img style="margin-bottom:10px" onclick="(window.location.href='design_detail.html?id=${wentuList[i].id}')" class="lazy" src="../img/blank.gif" data-echo="${QiNiu+wentuList[i].imgName}">

        ...
        <!-- 分页 -->
          <div class="block" style="overflow:hidden;">
			...
			<el-form :model="ruleForm" :rules="rules"   ref="ruleForm"  size='medium'    class="demo-ruleForm">
				...
				prop="title"
				>
					<el-input type="text"  placeholder="请输入描述内容" v-model.number="ruleForm.title" autocomplete="off">
...
export default {
    ...
    methods:{
        ...
        // 新增一条的接口
        addServer(pamas){
            postRequest(url.slideshowInsert, pamas).then(data=> {
                this.uploadNum=0
        ...
        this.$confirm('是否删除?', '提示', {
            ...
            type: 'warning'
                ...
                postRequest(url.slideshowDelete, {
                    ids:rows[index].id,
                    imgName:rows[index].imgName,
                }).then(data=> {
        ...
        initData(pamas){
            postRequest(url.slideshowGetSlideshowList, pamas).then(data=> {
                if(!getErr(data.data.code,data.data.message)) {
                    return
                }
